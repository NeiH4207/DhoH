...
def get_cum_prob(self, array):
    cum_prob = []
    for i in range(len(array)):
        if i == 0:
            cum_prob.append(array[i])
        else:
            cum_prob.append(array[i] + cum_prob[-1])
    return cum_prob

def get_random_id_from_cum_prob(self, cum_prob):
    r = uniform(0, 1)
    return bisect_left(cum_prob, r)

def wheel_select_id(self, pop, k=1):
    # normalize fitness
    min_fit = np.min([x[self.ID_FIT] for x in pop])
    max_fit = np.max([x[self.ID_FIT] for x in pop])
    norm_fit = np.cumsum([(x[self.ID_FIT] - min_fit) / (max_fit - min_fit) for x in pop])
    cum_prob = self.get_cum_prob(norm_fit)
    ids = []
    while(len(ids) < k):
        id = self.get_random_id_from_cum_prob(cum_prob)
        if id not in ids:
            ids.append(id)
    return ids
...
def current_to_pbest_weighted_mutation(population: np.ndarray,
    ...
    # 1. We find the best parent
    best_index = np.argsort(population_fitness)[:max(2, round(p*len(population)))].flatten()
